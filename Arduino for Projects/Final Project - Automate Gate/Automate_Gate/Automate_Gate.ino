...
#define LCD_ADD 0x27 // address of the i2c component (PCF8574)
...
{
  Serial.begin(9600);
  servo1.attach(3); // servo connect to PWM pin 3
  servo1.write(0);
                       ...
                       //in all of this process the RS pin on the LCD is OFF ('0')
{
  ...
  Wire.write ((val & 0xF0) | 0B00001000); //send 4 MSB to the LCD and enable pin on LCD OFF ('0')
  Wire.write ((val & 0xF0) | 0B00001100); //set the enable pin on LCD ON ('1')
  delay (5); //delay 5 mlliseconds
  Wire.write ((val & 0xF0) & 0B11111000); //set the enable pin on LCD OFF ('0') when the data is in the data pins of the LCD
                                          //sending the Less significant Bits (LSB)
  Wire.write (((val & 0x0F) << 4) | 0B00001000); //send 4 LSB to the LCD and enable pin on LCD OFF ('0')
  Wire.write (((val & 0x0F) << 4) | 0B00001100); //set the enable pin on LCD ON ('1')
  delay (5); // delay 5 milliseconds 
  Wire.write (((val & 0x0F) << 4) & 0B11111000); // set the enable pin on LCD OFF ('0') when the data is in the data pins of the LCD
                         ...
                         //the same process like the "lcd_cmd" function but the RS pin on the LCD is ON ('1')
{
  ...
  Wire.write ((val & 0xF0) | 0B00001001);
  Wire.write ((val & 0xF0) | 0B00001101);
  delay (5);
  Wire.write ((val & 0xF0) & 0B11111001);

  Wire.write (((val & 0x0F) << 4)| 0B00001001);
  Wire.write (((val & 0x0F) << 4)| 0B00001101);
  delay (5);
  Wire.write (((val & 0x0F) << 4)& 0B11111001);
